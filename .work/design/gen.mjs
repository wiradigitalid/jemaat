...
const artboards = [
  ...
  { file: 'AdminRolePicker.dc.html', title: 'W28 Choosing a serving role', x: 0, y: 1954, w: 1440, h: 900, page: 'page-8' },
  { file: 'AdminHousehold.dc.html', title: 'W26 A household', x: 1520, y: 1954, w: 1440, h: 1040, page: 'page-8' },
  { file: 'AdminGroups.dc.html', title: 'W24 Care groups', x: 0, y: 3074, w: 1440, h: 900, page: 'page-8' },
  { file: 'AdminDepartments.dc.html', title: 'W27 Departments and serving roles', x: 1520, y: 3074, w: 1440, h: 900, page: 'page-8' },
  { file: 'AdminDepartmentsEmpty.dc.html', title: 'W29 No departments yet', x: 3040, y: 3074, w: 1440, h: 900, page: 'page-8' },
  { file: 'AdminChurch.dc.html', title: 'W25 The church record', x: 0, y: 4154, w: 1440, h: 900, page: 'page-8' },
  { file: 'AdminScope.dc.html', title: 'S12 The register, alone', x: 0, y: 5134, w: 1440, h: 1020, page: 'page-8' },
  { file: 'AdminModel.dc.html', title: 'S14 Household, family, and the address', x: 0, y: 6274, w: 1440, h: 1300, page: 'page-8' },
  { file: 'AdminServing.dc.html', title: 'S15 Departments, roles, and interest', x: 0, y: 7694, w: 1440, h: 900, page: 'page-8' },
  { file: 'AdminRelease.dc.html', title: 'S13 What ships: the register', x: 0, y: 8714, w: 1440, h: 1150, page: 'page-8' },
];

const servingNote = {
  id: 'n-serving', x: 1520, y: 7694, w: 540, page: 'page-8',
  text: 'ITERATION 46 - A FLAT LIST WAS HIDING A HIERARCHY\nAreas of involvement was five words - Music, Multimedia, Prayer, Teaching, Visitation - typed straight into a person. The owner asked for the shape underneath, and it is two levels: A DEPARTMENT HOLDS SERVING ROLES, AND A PERSON IS INTERESTED IN A ROLE.\n\nThe flat list was worse than it looked. Nobody is interested in Music; they are interested in singing, or in playing keyboard, or in running the projector - and a coordinator who reads Music beside forty-nine names still has to phone all forty-nine to find out which. Two levels turn a label into a list a person can act on.\n\nTHE PICKER IS WHERE THIS IS EASY TO BUILD WRONGLY, which is why W28 exists as a state rather than a sentence. The department is a HEADING, not an option: it groups the results and it cannot be chosen. Search matches both levels, so typing a department name surfaces its roles, and the heading itself stays inert.\n\nAND ONE DETAIL THAT ONLY SHOWS UP IN A LIST: role names repeat across departments. Coordinator exists in four of them. So a chosen role always carries its department - Music / Coordinator - or the chip is worthless the moment it is read anywhere else.\n\nTHE LINE THIS MUST NOT CROSS. It. 5 named serving as its own axis - teams, slots, dates - and S1 cut all four of those screens to Later. What arrives here is the CATALOGUE and the WISH, and no schedule. The distance between an interest and a roster is one table, and building it early would quietly restore the largest cut on the release map.\n\nDeactivate rather than delete, for the same reason the register never deletes a person: interests already point at the row, and one day a roster will too.\n\nCORRECTED IMMEDIATELY AFTER: I PROPOSED A SEED LIST AND THE OWNER REMOVED IT. Six departments and twenty-four roles, offered on the argument W11 makes about the register - better to delete what you do not have than to invent from an empty screen. That argument does not transfer, and the difference is who owns the vocabulary. A household is a household in any church. A serving role is whatever THIS church calls it: pemusik, penyambut, kolektan, operator. Seeded rows nobody chose become rows the office has to clean, and an unused role keeps filling the picker for years.\n\nSo the catalogue ships empty and W29 is the screen a church actually meets first. The cost is real and stated on the sheet rather than hidden: until the first role exists, the interest field on W22 has nothing to offer and says so.',
};

const modelNote2 = {
  id: 'n-household', x: 1520, y: 6274, w: 540, page: 'page-8',
  text: 'ITERATION 45 - ONE WORD DOING TWO JOBS\nThe owner asked it plainly: is a household who lives in one house, or is it a family with a head? Forty-four iterations used one word for both and never noticed, because the sample data was a nuclear family at one address - the case where the two facts happen to coincide.\n\nTHEY ARE DIFFERENT FACTS AND THEY CHANGE AT DIFFERENT TIMES. A household is an address with people in it; it changes when somebody moves. A family is who those people are to each other; it survives a move. A daughter who marries out stops being in the household and does not stop being a daughter.\n\nWHAT SHIPS IS THE HOUSEHOLD, with a relationship written on each occupant - head, wife, son, daughter-in-law, grandchild, boards here. That is the shape of a Kartu Keluarga, which is what a church already prints and what a visitation team already uses. Kinship BETWEEN households is not a table yet, and W26 gets away without one through a Moved out row that says where she went: the link is kept, the graph is not.\n\nTHE ESCAPE HATCH IS THAT TWO HOUSEHOLDS MAY SHARE AN ADDRESS. A married son living in his parents house is two households under one roof, not one household of six - each with its own head, each getting its own family card. That single allowance is what lets the register hold family and household apart without modelling kinship at all.\n\nAnd the head is a role, not a rank: a widow living alone is the head of her own household, and nothing in the product hangs a permission on it.\n\nTHE PERSON GREW FROM SIX FIELDS TO ELEVEN, so the six fields and no more position is retired rather than quietly broken. A longer form is abandoned more often, so the phone keeps a four-field fast path with the rest behind More details - the form a volunteer fills standing in a hall is not the form the office fills on a Tuesday.\n\nThe address stays on the household, which is decision 6 unchanged. The person-level override exists for the student in a dorm and the worker in another city, and it is empty on almost every record.\n\nITERATION 46 CLOSED THE HALF THIS LEFT OPEN: not everyone under a roof is kin. A friend, a distant relative, a household helper. The relation field is not a kinship field at all - it is HOW A PERSON IS ATTACHED TO THIS HOUSEHOLD - so one list covers both, and whether they count as family is derived from the value rather than asked as a second question.\n\nThe vocabulary is already in every Indonesian home: a Kartu Keluarga carries famili lain and pembantu, so the office recognises the list instead of learning it. W26 shows the two groups as two blocks - Family, then Also lives here - which is the whole answer visible at a glance.\n\nAnd the helper is the case that proves the model: she is IN THE HOUSEHOLD AND NOT IN THE FAMILY. She belongs on the visitation list, she does not belong on the family card, and her own family is in Sukabumi where this register cannot see them and should not pretend to.',
};

const webNote = {
  id: 'n-admin', x: 1520, y: 5134, w: 540, page: 'page-8',
...
const webShipNote = {
  id: 'n-adminship', x: 1520, y: 8714, w: 540, page: 'page-8',
  text: 'RE-DERIVED TWICE, AND THE NUMBERS MOVED AGAIN\nThis column was rebuilt for one surface at it. 43 and again for one role at it. 44. It moves a third time here, from sixteen routes to seventeen and from six capabilities to seven, because the household earned its own screen and the map is a capability rather than a field. Every number on the sheet is meant to be checked against the column beside it - it. 22 counted 32 that was 31, and a number nobody checks is a number that gets planned against.\n\nFIVE HOLES CLOSE, all of them old. The office could not sign in, named at it. 16 and carried open for twenty-six iterations. Time zones were named on W12 and applied nowhere. Creating a person was mobile-only, which left W2 Add button pointing at an app screen. Care groups could only arrive through the Excel import. And household was doing two jobs under one word.\n\nWhy all five surface together is worth naming: they are the holes that only bite when THE OFFICE IS THE WHOLE PRODUCT. Nobody noticed while a leader and a member were carrying half the flows.\n\nTHE MAP IS THE ONE GENUINELY NEW CAPABILITY. Tiles, address search, a stored coordinate and a draggable pin. It is last in the build order on purpose: everything in the register works without it, just worse, and a visitation team is the only party that needs it.\n\nWHAT THE APP INHERITS. Every M, O and P artboard stays valid against the same API. The app is the other three roles arriving at a register that already holds real data - the easier order to build in and the harder one to sell, because the congregation sees nothing until the second release.\n\nAnd one thing left deliberately unfixed: older sheets still say Community Member in their prose. The pill changed everywhere at once because it lives in one function; the prose did not, and pretending otherwise on this sheet would be the same stale map in a smaller font.',
};

const topNavNote = {
  id: 'n-adminrow', x: 1880, y: 0, w: 460, page: 'page-8',
  text: 'THE COPY RULE, AND WHAT IT EXPOSED\nSet by the owner at it. 45: no design talk on the screens. No iteration numbers, no screen codes, no rationale panels, no not-yet-in-this-release boxes. A screen shows what the office would see and nothing else. Every reason now lives in these notes and on S12, S13 and S14, which are sheets rather than product.\n\nIT WAS NOT A COSMETIC CHANGE. Three of the desktop screens were half rationale by area, and stripping the rails left them visibly empty - which was the honest answer to a question I had not asked: a screen that only looked finished because a design note sat beside it was not finished.\n\nSo the real UI had to grow to fill the frame, and everything it grew is work an office actually does. W24 gained the queue of twelve people who asked for a care group and have not been placed. W25 gained the administrator list and what the register currently holds. W26 exists at all. The rule found missing product, not missing prose.\n\nFOUR PHONE SCREENS IN AN OFFICE RELEASE, and they are here because of S9 rather than because of a member: most small Indonesian churches have no paid staff and no desk. The split follows the rule S9 set - not who you are, but how big the job is. Adding one person follows a volunteer around a building; the table, the import and the merge stay on a desktop and are better there.\n\nW20 is the only dead end in the release and every administrator meets it once, because the sign-in link arrives in WhatsApp and opens inside WhatsApp browser, where there is nothing to save it to. W21 is drawn already saved to a home screen, so the three read as one story: sign in, get stuck, get out.',
};
